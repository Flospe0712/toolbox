...
export default function AnalyticsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [
        igRes, igReelsRes, igStoriesRes, igStatsRes,
        twRes, twStatsRes, liRes, liStatsRes, ytStatsRes,
        ytPostsRes, ytTimelineRes,
      ] = await Promise.all([
        fetch(`/api/metricool?endpoint=instagram_posts&from=${fromStr}&to=${toStr}`),
        fetch(`/api/metricool?endpoint=instagram_reels&from=${fromStr}&to=${toStr}`),
        fetch(`/api/metricool?endpoint=instagram_stories&from=${fromStr}&to=${toStr}`),
        fetch(`/api/metricool?endpoint=instagram_stats&date=${todayYmd}`),
      ...
      const [
        igData, igReelsData, igStoriesData, igStatsData,
        twData, twStatsData, liData, liStatsData, ytStatsData,
        ytPostsData, ytTimelineData,
      ] = await Promise.all([
        igRes.json(), igReelsRes.json(), igStoriesRes.json(), igStatsRes.json(),
      ...
      setData({
        igPosts: igData?.data ?? [],
        igReels: igReelsData?.data ?? [],
        igStories: igStoriesData?.data ?? [],
        igStats: igStatsData && !igStatsData.error ? igStatsData : null,
        twPosts: twData?.data ?? [],
